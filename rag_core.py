...
# Clients
# =========================
def _clean_base_url(raw: str | None) -> str | None:
    """
    Defensive sanitation for deployment env vars.
    Handles accidental surrounding quotes, whitespace, or empty strings.
    """
    if raw is None:
        return None
    s = str(raw).strip()
    if not s:
        return None
    # strip surrounding single/double quotes (common Vercel copy/paste issue)
...
def get_clients() -> Tuple[OpenAI, Any]:
    openai_client = OpenAI(
        api_key=os.environ["OPENAI_API_KEY"],
        base_url=_clean_base_url(os.environ.get("OPENAI_BASE_URL")),
    )
    pc = Pinecone(api_key=os.environ["PINECONE_API_KEY"])
    index = pc.Index(os.environ.get("PINECONE_INDEX", "ted-rag"))
    return openai_client, index
...
def ask(question: str) -> Dict[str, Any]:
    ...
    # ✅ Change (D): return a substantially larger (or full) chunk in the context.
    # We keep it bounded to MAX_CONTEXT_CHARS to avoid huge payloads.
    context_out = [
        {
            "talk_id": m["metadata"]["talk_id"],
            "title": m["metadata"]["title"],
            "chunk": m["metadata"]["chunk"][:MAX_CONTEXT_CHARS],
            "score": float(round(m["score"], 4)),
        }
        for m in chosen
    ]
